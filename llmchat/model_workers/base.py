...
from llmchat.settings import LOG_PATH
...
import fastchat
import asyncio
from llmchat.utils import get_model_worker_config
...
class ApiConfigParams(BaseModel):
    ...
    def load_config_not_declare(self, worker_name: str):
        """
        加载config，无需字段中包含config属性
        """
        self.worker_name = worker_name
        if config := get_model_worker_config(worker_name):
            for n in config:
